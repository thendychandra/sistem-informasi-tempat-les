...
(function(undefined) {
    ...
    var args = {
        ...
        resultFunction: function(res) {
            ...
            scannedQR[txt] = res.format + ": " + res.code;
                ...
                // xhr.open("POST", "./model/proses.php", true);
                ...
                //     type: "POST",
                //     url: "./model/proses.php",
                //     data: 'qr='+res.code,   //with the page number as a parameter
                //     dataType: "html",   //expect html to be returned
                //     async: true,
                // });
                var qr = res.format + ": " + res.code;
                $.ajax({
                    type: "POST",
                    url: "./model/proses.php",
                    dataType: "json",
                    ...
                    success : function(data){
                        if (data.code == "200"){
                            $(".display-msg").html("<div class='alert alert-success'><strong>Absen berhasil. Terimakasih "+data.msg+".</strong></div>");
                            $(".display-msg").css("display","block");
                            //hilangkan image warning sidebar dan matikan kamera
                            const absen_sidebar = document.querySelectorAll("#absen_sidebar");
                            absen_sidebar[1].innerHTML = "<a href='qr_absen'><i class='fas fa-qrcode'></i> Absen Qr-Code</a>";
                            grabImg.classList.add("disabled");
                            scannedQR[txt] = "Stopped";
                            decoder.stop();
                        } else {
                            $(".display-msg").html(data.msg);
                            $(".display-msg").css("display","block");
                        }
                    }
                });
    ...
    play.addEventListener("click", function() {
        if (!decoder.isInitialized()) {
            ...
            // kode tambahan untuk membalik fliphorizontal - bagian if
            document.getElementById("flipHorizontal").setAttribute("checked", "_self");
            if (decoder.isInitialized()) {
            ...
            scannedQR[txt] = "Scanning ...";
            decoder.play();
            // kode tambahan untuk membalik fliphorizontal - bagian else
            document.getElementById("flipHorizontal").setAttribute("checked", "_self");
            if (decoder.isInitialized()) {
                var value = flipHorizontal.checked;
                if (value) {
