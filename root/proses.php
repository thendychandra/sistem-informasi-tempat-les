...
include '../lib/db/dbconfig.php';
	if (isset($_POST['add_pbXia1Zasww12Q'])) {
		...
		if ($email==="" || $pwd==="" || $nama ==="") {
			header("location:./view/adm/add_pb.php?st=4");
		} else {
			if ($cek==0) {
				if ($statement = $conn->prepare($sql_user) AND $statement1 = $conn -> prepare($sql_detail)) {
						...
						if ($statement->execute() && $statement1->execute()) {
							header("location:./view/adm/add_pb.php?st=1");
						} else {
							header("location:./view/adm/add_pb.php?st=2");
						}
				}
			} else {
				header("location:./view/adm/add_pb.php?st=3");
			}
		}
		$conn->close();
	} else {
