<h3 class='page-header'>Daftar Siswa Athena</h3>
<?php 
if (isset($_GET['st'])) {
        if ($_GET['st']==1) {
        ...
        if (isset($_GET['id_siswa'])) {
            ...
            echo(
                '<form action="siswa" method="post">
                    ...
                    <input id="search" type="text" name="cari_nama"><input type="submit" value="Cari">
                </form>'
            );
$limit = 10;
$start = 1;
...
if(isset($_POST['cari_nama'])){
    $search = $_POST['cari_nama'];
    $q = "SELECT * FROM detail_user WHERE name_user LIKE '%$search%' ORDER BY id_user ASC";
}else{
...
if(isset($_POST['cari_nama'])){
    $search = $_POST['cari_nama'];
    $q = "SELECT * FROM detail_user INNER JOIN user ON detail_user.id_user = user.id_user WHERE name_user LIKE '%$search%' ORDER BY sklh_user ASC LIMIT $limitvalue, $limit";
}else{
    $q = "SELECT * FROM detail_user INNER JOIN user ON detail_user.id_user = user.id_user ORDER BY sklh_user ASC LIMIT $limitvalue, $limit";
}
...
if ($cek_page->num_rows != 0) {
    ...
    if ($r->num_rows!==0) {
            echo "<table class='table table-striped' style='width:70%'>
                ...
                if ($hitung >= $limit) {
                    echo "<hr style='margin-top: -13px'><ul class='pagination' style='margin-top: -11px'>";
                   if($page!= 1){
                        $pageprev = $page - 1;
